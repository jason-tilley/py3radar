...
def merge_clusters(img_class_flat, centers, clusters_list):
    ...
    if below_threshold:
        ...
        clusters_list = np.append(clusters_list, np.arange(start, end))
...
def initial_clusters(img_flat, k, method="linspace"):
    ...
    assert method in methods_availables
    #"ERROR: method %s is no valid." "Methods availables
    #%s" % (method, methods_availables)
    if method == "linspace":
